...
class WritebackSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  ...
  def testVreg(dut: Writeback): Unit = {
    ...
    for(i <- 0 until SUBVECTORS_PER_VREG) {
      for(j <- 0 until NUM_PROCELEM) {
        dut.io.in.res(j).poke(inputs(i)(j))
      }
      dut.io.in.dest.poke(dests(i))
      dut.io.in.valid.poke(true.B)
      dut.io.in.reduce.poke(false.B)
      dut.clock.step()
      print(s"i=$i\n")
    ...
    for(i <- 0 until VREG_DEPTH) {
      ...
      dut.io.out.wrData(i).expect(inputs(sv)(j))
      dut.io.out.rd.reg.expect(rd.U)
      dut.io.out.rd.rf.expect(VREG)
    }
  }
  ...
  def testSingle(dut: Writeback, rf: RegisterFileType.Type, reduce: Boolean = false): Unit = {
    ...
    for (i <- 0 until NUM_PROCELEM) {
      dut.io.in.res(i).poke(inputs(i))
    }
    dut.io.in.valid.poke(true.B)
    dut.io.in.reduce.poke(reduce.B)
    ...
    if(reduce) {
      ...
      for(i <- 0 until NUM_PROCELEM) {
        dut.io.out.wrData(i).expect(res)
      }
    } else {
      for (i <- 0 until NUM_PROCELEM) {
        dut.io.out.wrData(i).expect(inputs(i))
      }
    }
    for (i <- NUM_PROCELEM until VREG_DEPTH) {
      dut.io.out.wrData(i).expect(0.S)
    }
    dut.io.out.rd.rf.expect(rf)
    dut.io.out.rd.reg.expect(rd.U)
