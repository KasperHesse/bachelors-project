...
class TopLevelStageSpec extends FlatSpec with ChiselScalatestTester with Matchers {
   ...
   * @param memInit The location of memory initialization files generated by a previous simulation.
   *                If this parameter is set, the memory init file from a previous simulation is used
   *                to initialize memory and registers. Must be of the format "testname/hash"
   */
  def testFun(filename: String, timeout: Int = 500, annos: AnnotationSeq = Seq(), dumpMemory: Boolean = false, memInit: String = ""): Unit = {
    ...
    val wordsPerBank = if(memInit.isEmpty) {
      SynthesisMemInit("src/resources/meminit")
    } else {
      val s = memInit.split('/')
      val wbp = SimulationMemInit(s(0), s(1), "src/resources/meminit")
      println(s"Initialized memory with data from test $memInit")
      wbp
    }
  ...
  it should "execute a simple program" in {
    testFun("simple", dumpMemory = true, memInit = "adfg/eb7bb0")
  }
